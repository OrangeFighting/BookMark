...
public class DiskLruCache {
    ...
    private final int maxCacheItemSize = 500; // 500 item default
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        long usableSpace=Utils.getUsableSpace(cacheDir);
        if(usableSpace<maxByteSize)
        {
        	maxByteSize=usableSpace/2;
        }
//        if (cacheDir.isDirectory() && cacheDir.canWrite()
//                && Utils.getUsableSpace(cacheDir) > maxByteSize) {
//            return new DiskLruCache(cacheDir, maxByteSize);
//        }
        Log.e(TAG,"USABLE space:"+Utils.getUsableSpace(cacheDir)+"   max:"+maxByteSize);
        if (cacheDir.isDirectory() && cacheDir.canWrite())
        {
            return new DiskLruCache(cacheDir, maxByteSize);
        }
